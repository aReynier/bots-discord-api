...
import {
  IPromotionsServiceToken,
  IPromotionsService,
} from 'src/promotions/interfaces/promotion.interface';
import {
  IMembersServiceToken,
  IMembersService,
} from 'src/members/interfaces/member.interface';
import {
  IRolesServicesToken,
  IRolesService,
...
import {
  IGuildsServiceToken,
  IGuildsService,
} from 'src/guilds/interfaces/guild.interface';
import {
  IChannelsServiceToken,
  IChannelsService,
} from 'src/channels/interfaces/channel.interface';
import {
  IDiscordUsersServiceToken,
  IDiscordUsersService,
} from 'src/discord-users/interfaces/discord-user.interface';
...
import {
  PromotionSignatureDto,
  MembreDto,
  PromotionsSignatureResponseDto,
  RoleDto,
} from './dto/promotion-signature.dto';
...
export class SignatureService {
  ...
  async getAllPromotions(): Promise<PromotionsSignatureResponseDto> {
    ...
    return {
      promotions: promotions.map((promotion) =>
        this.mapPromotionToDto(promotion),
      ),
    };
  }
  ...
  private mapPromotionToDto(promotion: Promotion): PromotionSignatureDto {
    ...
    const projectManager = promotion.managers?.find(
      (member) => member.communityRole === 'ProjectManager',
    );
    const trainers =
      promotion.managers?.filter(
        (member) => member.communityRole === 'Trainer',
      ) || [];
    const learners =
      promotion.followers?.filter(
        (member) => member.communityRole === 'Learner',
      ) || [];
   ...
   */
  async generateTestPromotionSignature(): Promise<{
  ...
  async createTestData(): Promise<PromotionsSignatureResponseDto> {
    ...
    try {
      ...
      const existingCourse = await this.courseRepository.findOne({
        where: { nameCourse: 'Développeur Web' },
      });
      ...
      if (!existingPromotion) {
        ...
        promotion.uuidCourse = course.idCourse;
      ...
      if (promotionWithManagers && promotionWithManagers.managers) {
        ...
        if (promotionWithManagers.managers.length === 0) {
          promotionWithManagers.managers = [
            projectManager,
            ...trainers,
          ] as any[];
  ...
  async addSpecificPromotionData(): Promise<PromotionsSignatureResponseDto> {
    ...
    try {
      ...
      for (let i = 0; i < learnerDiscordIds.length; i++) {
        ...
        if (discordId === '223812446312202251') {
          const yohanMember = await this.memberRepository.findOne({
            where: { uuidDiscord: discordId },
          });
      ...
      this.logger.error(
        `Échec de l'ajout des données de promotion: ${error.message}`,
        error.stack,
      );
      throw new Error(
        `Échec de l'ajout des données de promotion: ${error.message}`,
